...
const Navbar = () => {
  ...
  return (
    <nav className="w-full flex py-6 justify-between items-center navbar">
      <img src={logo} alt="bankly" className="w-[64px] h-[64px]" /> 
      <span className="font-poppins font-bold text-4xl text-white ml-2">Bankly</span> 

      <ul className="list-none sm:flex hidden justify-end items-center flex-1">
        {navLinks.map((nav, index) => (
          <li key={nav.id} className={`font-poppins font-normal text[16px] cursor-pointer text-white hover:text-secondary ${index === navLinks.length - 1 ? 'mr-0' : 'mr-10'}`}>
